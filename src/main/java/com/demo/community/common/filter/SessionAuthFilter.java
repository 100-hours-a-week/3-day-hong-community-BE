...
import com.demo.community.common.dto.authErrorCode;
...
public class SessionAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (cookies == null || cookies.length == 0){
            // 쿠키를 포함시키세요
            writeError(response, HttpServletResponse.SC_UNAUTHORIZED, authErrorCode.AUTH_NO_COOKIE, "cookie is missing");
            return;
        }
        ...
        for (Cookie c : cookies) {
            if ("JSESSIONID".equals(c.getName())){
                // 쿠키에 세션을 포함시키세요
                writeError(response, HttpServletResponse.SC_UNAUTHORIZED, authErrorCode.AUTH_NO_SID, "no SID in cookie");
                return;
            }
        }
        ...
        if (requestedSid != null && !sidValid){
            ...
            writeError(response, HttpServletResponse.SC_UNAUTHORIZED, authErrorCode.AUTH_INVALID, "inavliad access, redirect to login");
            return;
        }
        ...
        if (requestedSid != null && session == null){
            ...
            writeError(response, HttpServletResponse.SC_UNAUTHORIZED, authErrorCode.AUTH_EXPIRED, "session expired, redirect to login");
            return;
    ...
    private void writeError(HttpServletResponse response, int status, authErrorCode code, String message) throws IOException {
